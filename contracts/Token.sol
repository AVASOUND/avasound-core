...
contract Token is Ownable, ERC1155 {
    ...
    // Events
     event AlbumCreated(address indexed owner,uint256 masterNFTID,uint256 downloadTokensID,uint256 royaltyTokenID);
     event DownloadTokenPrice(address indexed owner,uint256 dateCreated,uint256 downloadTokenID,uint256 downloadTokenPrice);
     event TokensForSale(uint256 albumID,uint256 dateApproved,uint256 amount);

    constructor(address initialRoyaltiesReceiver) ERC1155("https://jjk1k8ehc6ws.usemoralis.com:2053/server/functions/api?_ApplicationId=5mgHy2kpLFIjmySx7yAAJMugLG4wVn1SsnOKZZhn&id={id}") {
    }
    function createAlbum(uint256 _numberOfDownloadTokens,
     uint256 _numberOfRoyaltyTokens,uint256 _downloadTokenPrice,uint256 _royaltyTokenPrice) external payable
     {
       ...
       emit AlbumCreated(msg.sender,masterNFTID,downloadTokensID,royaltyTokenID);
       emit DownloadTokenPrice(msg.sender,block.timestamp,downloadTokensID,_downloadTokenPrice);
